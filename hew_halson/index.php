...
// DB処理
include_once('db_config-local.php');
...
<body>
		...
		<div id="selectedQuantityModal" class="selected-quantity-modal">
			<form class="selected-quantity-modal__form">
				...
				<div class="selected-quantity-modal__content">
					...
					<select id="selectedQuantitySelect" class="selected-quantity-modal__select"></select>
	...
	<div id="orderArea" class="order-area">
		<form id="orderDetails" class="order-area__wrap">
			...
			<li class="order-area__details__item">
				<p class="order-area__details__item__title">小計 [合計個数]</p>

				<ul class="order-area__details__item__content">
					<li id="orderAreaSum" class="order-area__details__item__content__text"></li>
				</ul>
			</li>

				<li class="order-area__details__item">
					<p class="order-area__details__item__title order-area__details__remove-disabled">お届け先（指定した住所にお届けのみ）<span><a href="">変更</a></span></p><ul class="order-area__details__item__content">
						<li class="order-area__details__item__content__text"><input type="text" value="ハルソン" disabled></li>
						<li class="order-area__details__item__content__text"><input type="text" value="1600023" disabled></li>
						<li class="order-area__details__item__content__text"><input type="text" value="東京都新宿区西新宿1丁目7-3" disabled></li></ul>
				</li>

				<li class="order-area__details__item">
					<p class="order-area__details__item__title order-area__details__remove-disabled">お支払方法（クレジットカード払いのみ）<span><a href="">変更</a></span></p><ul class="order-area__details__item__content">
						<li class="order-area__details__item__content__text"><input type="text" value="1234567891234567" disabled></li></ul>
				</li>

				<li id="orderAreaDetailsHideLi" class="order-area__details__item" style="display:none">
					<p class="order-area__details__item__title">お届け先・お支払方法の情報に変更した場合</p>
					
					<ul class="order-area__details__item__content">
						<li class="order-area__details__item__content__text">
							<label>
								<input type="checkbox" value="change">次回から変更した情報を使う
							</label>
						</li>
					</ul>
				</li>
	...
	<!-- お問い合わせ -->
	<div id="contactArea" class="login-area login-area--active">
		<div class="page-overlay login-area__overlay">
			<!-- お問い合わせエリア -->
			<?php if (!isset($_SESSION['id'])) : ?>
				<p class="contact-area__not-logged-in">お問い合わせはログイン後に使えます。</p>
	...
	<!-- マイページ -->
	<div id="mypageArea" class="mypage-area login-area--active">
		<div class="page-overlay login-area__overlay">
			<?php if (!isset($_SESSION['id'])) : ?>
				<p class="login-area__logined">マイページはログイン後に使えます。</p>

			<?php else : ?>
				<form class="login-area__form" action="login.php" name="loginForm" method="post" autocomplete="off">

					<p class="modal-title"><span id="myPageAreaModalTitle" class="modal-title__span"></span>様のマイページ</p>

					<table>
						<tr>
							<th><label for="mypage_id">メールアドレス：</label></th>
							<td><input id="mypage_id" type="email" disabled></td>
						</tr>
						<tr>
							<th><label for="mypage_name">名前：</label></th>
							<td><input id="mypage_name" type="text" disabled></td>
						</tr>
						<tr>
							<th><label for="mypage_kananame">フリガナ：</label></th>
							<td><input id="mypage_kananame" type="text" disabled></td>
						</tr>
						<tr>
							<th><label for="mypage_nickname">ニックネーム：</label></th>
							<td><input id="mypage_nickname" type="text" disabled></td>
						</tr>
						<tr>
							<th><label for="mypage_pass">パスワード：</label></th>
							<td><input id="mypage_pass" type="password" disabled></td>
						</tr>
						<tr>
							<th><label for="mypage_post">郵便番号：</label></th>
							<td><input id="mypage_post" type="text" disabled></td>
						</tr>
						<tr>
							<th><label for="mypage_address">住所：</label></th>
							<td><input id="mypage_address" type="text" disabled></td>
						</tr>
						<tr>
							<th><label for="mypage_credit">クレジットカード番号：</label></th>
							<td><input id="mypage_credit" type="text" disabled></td>
						</tr>
					</table>
				</form>
			<?php endif ?>
