...
function init() {
  ...
  // glbファイル読込(商品)
  for(let i = 0; i < jsonProductData.length; i++){
    const url = 'http://localhost/hew_211214_toppage/glb/' + jsonProductData[i].imageurl + '.glb';
    loader.load(
      url,
      (gltf) => {
  ...
  // jsonProductDataの各オブジェのnameプロパティの値を配列として取得
  const nameKeyOfJsonProductData = [];
  for(let i = 0; i < jsonProductData.length; i++) nameKeyOfJsonProductData.push(jsonProductData[i].name);
  ...
  function dblclickObj(e) {
    ...
    // クリックしたオブジェクトに対応するjsonProductDataの要素のインデックスを取得
    matchIndex =  nameKeyOfJsonProductData.findIndex(function(element) {
      return element == obj[0].object.parent.name;
    });
  ...
  function makeProductModal() {
    ...
    img.src = `img/${jsonProductData[matchIndex].imageurl}.png`;
    imgDiv.appendChild(img);

    const h2 = document.createElement('h2');
    h2.id = 'productModalTitle';
    h2.classList.add('product-modal__title');
    h2.innerText = jsonProductData[matchIndex].name;
    ...
    priceSpan.innerText = jsonProductData[matchIndex].price;
    ...
    quantitySpan.innerText = jsonProductData[matchIndex].quantity;
    ...
    descP.innerText = jsonProductData[matchIndex].explanation;
    ...
    for(let i = 1; i <= jsonProductData[matchIndex].quantity; i++){
  ...
  function addLocalStorage() {
    ...
    for(let i = 0; i < results.length; i++){
      ...
      if(result.value.name === jsonProductData[matchIndex].name){
        resultIndex = i;
        break;
      }
    }
    ...
    // 在庫数を超えてないかチェック
    if(resultIndex !== -1 && results[resultIndex].value.quantity == jsonProductData[matchIndex].quantity){
      viewMessageAsAddToCart(0.75, '在庫不足のため、カートに追加できません');
      return;
    }
    ...
    // value設定
    const value = {
      name: jsonProductData[matchIndex].name,
      price: jsonProductData[matchIndex].price,
      stock: jsonProductData[matchIndex].quantity,
      imageurl: jsonProductData[matchIndex].imageurl
    };
    ...
    if(resultIndex !== -1){
      selectedQuantity += results[resultIndex].value.quantity;
      selectedQuantity = Math.min(selectedQuantity, jsonProductData[matchIndex].quantity); // 在庫数を超えないようにする
    }
    ...
    viewMessageAsAddToCart(0.75, 'カートに商品が追加されました');
  }
  ...
  function viewLocalStorage() {
    ...
    for(let i = 0; i < results.length; i++){
      ...
      const img = document.createElement('img');
      img.src = `img/${result.value.imageurl}.png`;
      div.appendChild(img);
      ...
      select.addEventListener('change', () => {
        if(select.selectedIndex === 0){ // option[0]はカートから削除
          ...
          value.quantity = select.selectedIndex;
      ...
      for(let i = 0; i <= result.value.stock; i++){
        ...
        if(i === result.value.quantity) option.selected = true;
        select.appendChild(option);
      }
      sumquantity += result.value.quantity;
      ...
      const price = parseInt(result.value.price) * parseInt(result.value.quantity);
      sumPrice += price;
      p2.innerText = `${price}円`;
      div2.appendChild(p2);

      const div3 = document.createElement('div');
      div3.classList.add('cart-area__delete-btn');
      div3.innerText = '削除';
    ...
    sumQuantityArea.innerText = sumquantity;
  }
  ...
  //   // window.location.href = `http://localhost/hew_211214_toppage/decrement_quantity.php?stores_id=${phpGetStoreNum}&products_id=${matchIndex+1}`;
  // });
  ...
  function makeForumModal(e) {
    ...
    for(let i = 0; i < jsonForumData.length; i++){
      if(jsonForumData[i].product_name == jsonProductData[matchIndex].name) indexOfJsonForumData.push(i);
    }

    if(indexOfJsonForumData.length === 0){
      const p = document.createElement('p');
      p.classList.add('product-modal__forum-empty');
      p.innerText = 'この商品についての投稿は\nまだありません。';
    ...
    for(let i = 0; i < indexOfJsonForumData.length; i++){
      const idx = indexOfJsonForumData[i];
      ...
      goodCnt.innerText = jsonForumData[idx].good_cnt;
      goodWrap.appendChild(goodCnt);

      const date = document.createElement('p');
      date.classList.add('product-modal__forum-date');
      date.innerText = jsonForumData[idx].created_at;
  ...
  function openForumForm(e) {
    ...
    form.action = `forum.php?stores_id=${phpGetStoreNum}`;
    form.method = 'post';
    ...
    productsIdOfInput.value = jsonProductData[matchIndex].id;
  ...
  // カートに商品追加時、メッセージ出力
  const viewArea = document.getElementById('viewMessageArea');
  const viewAreaStyle = window.getComputedStyle(viewArea);
  ...
  function viewMessageAsAddToCart(end, message) {
    ...
    if(viewAreaStyle.display == 'none'){
      viewArea.innerText = message;
      viewArea.style.display = 'block';
    }

    const startOpacity = parseFloat(viewAreaStyle.opacity);
    ...
    requestAnimationFrame(startTime => {
      ...
      function update(timeStamp) {
        ...
        if(progress > 0) viewArea.style.opacity = startOpacity + diffOpacity * progress;
        ...
        if(progress < 1){
          ...
          if(end === 0){
            ...
            setTimeout(() => { canPreparedAddToCart = true; }, 500);
            return;
          }
          setTimeout(() => { viewMessageAsAddToCart(0); }, 2100);
        }
      }
    });
  }
