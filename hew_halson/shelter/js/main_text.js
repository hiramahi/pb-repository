...
function firstInit() {
  ...
  // glbファイル読込(商品)
  for(let i = 0; i < productsKeyArr.length - 250; i++){
  ...
  let intervalID = setInterval(() => { progressLoading(); }, 10);
  let dragControls = null; // ドラッグアンドドロップ処理

  function progressLoading() {
    // パーセント算出
    const percent = (wrap.children.length / (productsKeyArr.length - 250)) * 100;
    progressBarPercent.textContent = Math.round(percent);

    // 全ての3Dオブジェを配置後に実行
    if(percent === 100 && wrap.children.length === (productsKeyArr.length - 250)){
      clearInterval(intervalID);
      setTimeout(() => {
        document.getElementById('loadingArea').classList.add('loading-area-inactive');
        mapTick();
  ...
  console.log(defaultGnavList);
  console.log(cloneGnavList);
  ...
  loader2.load('js/threejs/helvetiker_bold.typeface.json', function(font){
